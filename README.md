...
| **[/e2e-pr-stabilizer](./skills/testing/e2e-pr-stabilizer/SKILL.md)** | Local-first stabilizer for Playwright E2E on one PR. Pulls Dash0 MCP spans (`git.pull_request_link`) as historical baseline, then iterates locally with `--trace=on` and the same OTel exporter. Double-gates every fix with `Skill('confidence','analysis')` ≥ 90 % on the root cause **and** `Skill('confidence','code')` ≥ 90 % on the diff, plus a static + live selector-existence check that refuses hallucinated locators. Commits only after 3 consecutive local passes; pushes once and watches one CI run as ratification. Refuses `.skip` / `.fixme` / `waitForTimeout`. Modes: `stabilize` (default) and `optimize` (report-only, ranks slow-action wins by measured ms saved). | `/` |
