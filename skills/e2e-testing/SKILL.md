...
- [`test-provenance-guard`](../test-provenance-guard/SKILL.md) — runs after
  Generator output to catch tests-by-construction.
- [`confidence`](../confidence/SKILL.md) — gate when the heal loop fails.
- [`holistic-analysis`](../holistic-analysis/SKILL.md) — if a flow is failing
  for reasons no test rewrite can fix, step back instead of patching.
- [`playwright-trace-analyzer`](../playwright-trace-analyzer/SKILL.md) —
