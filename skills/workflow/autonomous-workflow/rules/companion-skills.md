...
| `persistent-memory`  | 1     | Always — load accumulated workflow lessons before design (fast tier of the self-improvement loop) | `read aw-lessons`     | Remove invocation in [`phase-1-planning.md`](./phase-1-planning.md#lessons-read) |
...
| `persistent-memory`  | 4     | At stuck-loop escalation (cap hit / user escalation) — record the failing area and resolution | `write aw-lessons --auto` | Remove invocation in [`phase-4-testing.md`](./phase-4-testing.md#lessons-write) |
...
| `persistent-memory`  | 7     | End-of-run (CI green / user stop / post-merge bug) — record durable run lessons; check promotion | `write aw-lessons --auto` | Remove invocation in [`phase-7-ci-gate.md`](./phase-7-ci-gate.md#lessons-write) |
