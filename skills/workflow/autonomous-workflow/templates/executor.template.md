...
| ----- | ---------------------- | -------------------------------------------------------------------- | ---------------- |
| 3     | `persistent-memory`    | Load lessons for the area before implementing                        | `read aw-lessons` |
...
| 4     | `persistent-memory`    | At stuck-loop escalation — record failing area + resolution          | `write aw-lessons --auto` |
...
| 7     | `persistent-memory`    | End-of-run (CI green / user stop / post-merge bug) — record durable run lessons; check promotion | `write aw-lessons --auto` |
...
- **Read** lessons before implementing (`Skill("persistent-memory", "read aw-lessons")`)
  and treat matches for the area you touch as constraints.
- **Write** a lesson at Phase 4 stuck-loop escalation and at Phase 7 end-of-run
  (`Skill("persistent-memory", "write aw-lessons --auto")`): the failing area /
