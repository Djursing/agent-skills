...
The defaults above are tuned for the [`video-analyser`](../../video-analyser/SKILL.md) skill — the most expensive downstream consumer (per-token cost on a vision model). Two settings carry the weight:
