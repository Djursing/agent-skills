...
| `max-width`    | No       | `768` px                             | Output downscale ceiling. 768 px is the [`video-analyser`](../video-analyser/SKILL.md) Pareto knee — UI text stays legible, image tokens stay cheap (~786 tokens/frame on Sonnet). Crops smaller than this are not upscaled. Set `0` to disable downscale. |
...
[`video-analyser`](../video-analyser/SKILL.md) skill's Pareto knee — see
