...
| ------ | ----------------------------------------------- | ------------------------------------ | ------------------ |
| create | src/main/java/com/acme/orders/OrderService.java | Add create/cancel business rules     | Encapsulate domain logic |
| modify | src/main/java/com/acme/orders/OrderController.java | Wire new endpoint + validation     | Expose behavior via API |
...
| ----------- | -------------------------------------- | --------------------------------------------------- | ------------------------------ |
| unit        | rejects invalid order state transition | OrderServiceTest                                    | Business rule enforcement      |
| slice       | returns 400 for invalid request body   | OrderControllerTest (`@WebMvcTest`)                | API validation + error shape   |
| integration | persists and reloads aggregate         | OrderRepositoryIT (`@DataJpaTest` or Testcontainers) | Data access behavior         |
| integration | end-to-end create order flow           | OrderFlowIT (`@SpringBootTest`)                     | Full Spring wiring + contracts |
```

### 2e. Define verification commands

| When           | Command (Java/Spring defaults)                    |
| -------------- | ------------------------------------------------- |
| After edit     | `./mvnw -q -DskipTests compile`                  |
| Focused tests  | `./mvnw -Dtest=ClassName#method test`            |
| Before PR      | `./mvnw verify`                                  |

Check `pom.xml`, Maven profiles, and CI config to determine exact commands.
