...
**Complexity is primary; file count is the tie-breaker.**

| # | Question                                                                                  | If yes ->     | If no ->     |
| - | ----------------------------------------------------------------------------------------- | ------------- | ------------ |
| 1 | Is this task architectural / cross-cutting / design-heavy?                               | **Full Mode** | go to next   |
| 2 | Does this task involve unfamiliar code or domain areas?                                  | **Full Mode** | go to next   |
| 3 | Is the change touching 4+ files OR 2+ packages?                                          | **Full Mode** | **Lite Mode** |

| Mode     | Artifacts Required  |
| -------- | ------------------- |
| **Full** | **YES - MANDATORY** |
| **Lite** | No                  |
...
**State your mode selection explicitly before proceeding** using:
...
- **Stuck-loop has a mode-aware limit**: 3 iterations (Lite) / 5 iterations
  (Full) on the same failing area triggers `Skill("confidence", "bug-analysis")`
  and auto-replan or escalation.
...
| 4     | Testing                    | Autonomous    | Iterate until tests pass (mode-aware 3/5 cap) |
...
Phase 3: Implementation (Full: follow plan.md, update Progress Log)
    ...
    | (code complete)
Phase 4: Testing & Iteration <- mode-aware cap per failing area (3 Lite / 5 Full)
    | At cap: confidence(bug-analysis) -> auto-replan or escalate to user
