...
- **`Bash`** — `gw add` / `git worktree add`, `cd`, `./mvnw -q -DskipTests compile`
  for Java/Spring projects (or `npm install` / `pnpm install` for Node projects),
  then verify the worktree builds before declaring it ready,
