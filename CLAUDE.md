...
- `fix-bug` — Single-bug counterpart to `batch-linear-tickets`. Takes any starting evidence (Dash0 span / log / web event URL, raw stack trace, error message, code pointer `file:line`, screen recording, or free-text symptom), classifies it, resolves it (via Dash0 MCP, `/video-analyser`, stack-trace parsing, or codebase grep), maps it to source, delegates root-cause analysis to `holistic-analysis` (which itself runs `confidence(bug-analysis)`), and gates the proposed fix at >= 90%. At >= 90% confidence it hands off to `aw-planner` + `aw-executor` (autonomous-workflow namespace) to ship a draft PR; below 90% it returns the proposal for human review. Documents the future shape where `linear-ticket-investigator` becomes a Linear input adapter and `/batch-linear-tickets` becomes a thin batching wrapper around `/fix-bug`
