...
This fork is tuned for **Java + Spring Boot + Maven** as the default stack:
`pom.xml` projects use `./mvnw` commands for fast checks, test iteration, and
pre-PR verification (`./mvnw verify`). TypeScript/Node examples remain as
secondary guidance for non-Java repositories.
...
Mode selection uses three questions in order (complexity first, file count as
tie-breaker):

1. Is the task architectural / cross-cutting / design-heavy? -> Full
2. Is the code/domain unfamiliar? -> Full
3. If still simple: does it touch 4+ files or 2+ packages? -> Full, else Lite
