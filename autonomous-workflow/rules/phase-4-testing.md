...
| Changed                | Test type                       |
| ---------------------- | ------------------------------- |
| Spring MVC controllers | Web layer tests (`@WebMvcTest`) |
| Spring Data repositories | Data layer tests (`@DataJpaTest`) |
| Spring services        | Unit tests (JUnit + Mockito)    |
...
./mvnw test
# Or scoped to a class/method:
./mvnw -Dtest=OrderServiceTest#rejectsInvalidTransition test
...
class OrderControllerTest {
  ...
  @Test
  void createsOrderForValidPayload() throws Exception {
    // ...
  }
}
```
...
# If the project separates checkers, include them too
# ./mvnw test
# ./mvnw checkstyle:check spotbugs:check
```

The project's full verification gate must pass before exiting Phase 4.
