...
| Dependencies installed                  | per project (`./mvnw -q -DskipTests compile`, `pnpm i`, `go mod tidy`) |
...
Run the project's fastest type-check / compile loop. Java/Spring + Maven default first:

```bash
./mvnw -q -DskipTests compile # Java/Spring compile check
go vet ./...                  # Go vet
...
```bash
# Tests related to changed code (Java/Spring default)
