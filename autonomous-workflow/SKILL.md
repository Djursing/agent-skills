...
| 2     | `gw add`, `gw cd`, install deps (`./mvnw -q -DskipTests compile` for Maven), `Skill("aw-create-plan")` inside worktree |
| 3     | Code per `plan.md` → companions per task type (`tdd`, `ux`) → fast-check after each edit (`./mvnw -q -DskipTests compile`); `code-quality(code)` once at end |
| 4     | Run tests (focused JUnit loops then `./mvnw verify`) → iterate (mode-aware cap: 3 Lite / 5 Full) → `confidence(bug-analysis)` then escalate |
...
| 3     | Code, commit                                    |
| 4     | Test, fix failures (mode-aware cap applies; Lite cap = 3) |
...
- (THIS IS IMMENSELY IMORTANT AND SHOULD ALWAYS BE USED!!!) Flatpay Ai-guidelines -> read the CLAUDE.md file in ~/.agent/ai-guidelines/CLAUDE.md